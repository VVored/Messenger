    ...
    public class ChatHub : Hub
    {
        ...
        public async Task JoinBasicGroups()
        {
            await _context.ChatMembers
                .Where(cm => cm.UserId == int.Parse(Context.User.FindFirst(ClaimTypes.NameIdentifier).Value))
                .ForEachAsync(async cm => { await Groups.RemoveFromGroupAsync(Context.ConnectionId, cm.ChatId + "Notification");});
            await _context.ChatMembers
                .Where(cm => cm.UserId == int.Parse(Context.User.FindFirst(ClaimTypes.NameIdentifier).Value))
                .ForEachAsync(async cm => { await Groups.AddToGroupAsync(Context.ConnectionId, cm.ChatId + "Notification");});
        }
        ...
        public async Task JoinChat(string chatId)
        {
            var chat = await _context.Chats
                .Where(c => c.ChatId == int.Parse(chatId))
                .FirstOrDefaultAsync();
            var user = await _context.Users
                .Where(u => u.UserId == int.Parse(Context.User.FindFirst(ClaimTypes.NameIdentifier).Value))
                .FirstOrDefaultAsync();
        ...
        public async Task LeaveChat(string chatId)
        {
            var chat = await _context.Chats
                .Include(c => c.Members)
                .FirstOrDefaultAsync(c => c.ChatId == int.Parse(chatId));
            var member = await _context.ChatMembers
                .Include(cm => cm.User)
                .FirstOrDefaultAsync(cm => cm.UserId == int.Parse(Context.User.FindFirst(ClaimTypes.NameIdentifier).Value) & cm.ChatId == int.Parse(chatId));
